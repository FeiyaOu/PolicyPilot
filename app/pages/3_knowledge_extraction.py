...
def main():
    ...
    if st.button("提取知识", type="primary", disabled=not conversation):
        ...
        if result.status == KnowledgeExtractionStatus.FAILED:
            ...
            st.session_state.pop("lc_extraction_result", None)
            return
        ...
        # Store in session state so checkboxes and append button survive reruns
        st.session_state.lc_extraction_result = result

    # ── Show results from session state (persists across reruns) ─────────────
    if "lc_extraction_result" not in st.session_state:
        return

    result = st.session_state.lc_extraction_result
    ...
    if not CHUNKS_PATH.exists():
        ...
        if st.button(f"追加 {len(selected_items)} 个知识点到 chunks.jsonl", type="secondary"):
            ...
            if new_chunks:
                ...
                st.success(
                    f"✅ 已追加 {len(new_chunks)} 个知识点到 chunks.jsonl（现共 {len(all_dicts)} 条）。"
                    " 请在主页重新构建知识库以使新内容生效。"
                )
            else:
